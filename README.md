...
- Completed `have hOr := ...; cases hOr with ...` rewrite in `tryDecompCasesOn`: when the `casesOn` discriminant is a non-fvar application, we now emit a `have`-bound name and case on that, instead of inlining the big term. This unblocked `Sum/Int/Sum.lean` lines 55 and 81, which now decompile cleanly (discriminant `Lean.Grind.em _` → `have hOr : ... ∨ ¬... := by lia; cases hOr with ...`). It did **not** unblock `Unbundled/Int.lean` L76/L79/L91/L47 because in those the discriminant is itself an unhandled `Eq.mp (Lean.Grind.not_eq_prop ...) (mt ... hp)` chain that the inner `decompileOrExact` cannot decompose structurally, so it falls through to exact-fallback which trips the node-count guard.
- Diagnostic probe on `abs_lt_one_iff` (`Int.lean` L76) via `showtermexpanded grind` shows the proof structure is `Classical.byContradiction (fun h => Or.casesOn (Lean.Grind.of_eq_eq_true ...) <branchL> <branchR>)` where the `of_eq_eq_true` scrutinee contains a chain of `Eq.mp (Lean.Grind.not_eq_prop p q) (mt X hp)` and `X` itself is `congrFun' (congrArg Iff (Eq.trans (Lean.Grind.Int.lt_eq ...) (Int.Linear.norm_le ...))) (a = 0)` composed with `Lean.Grind.iff_eq`. The leaves of each `Or` branch are covered by `tryDecompAbsLeaf` + `lia`, but the *outer* `Or.casesOn` has no structural handler for the `of_eq_eq_true`-wrapped `Eq.mp` chain, so it falls to the exact fallback and hits the size guard. Raising the guard does not help because the resulting `exact <16k-node-term>` then runs into the top-level character-length limit and a re-elaboration slowdown (`lake env lean` on L79 did not complete within 12 minutes).
...
- **Handler for `Lean.Grind.of_eq_eq_true` + the `Eq.mp (Lean.Grind.not_eq_prop ...)` wrapper.** This is the next blocker for `Int.lean` L76/L79/L91. The scrutinee pattern is `Lean.Grind.of_eq_eq_true (eq_true (Eq.mp (...iff_eq chain...) (Eq.mp (not_eq_prop ...) (mt X hp))))`. A targeted handler could recognize this shape, emit `by_cases ha : a` / `by_cases hb : b` on the two disjuncts (`a = (|a| ≤ 0)`, `b = (¬(a = 0))` at L76) and discharge the impossible cross cases with `exact absurd h_eq_proof ...` or `lia`. This would fully bypass emitting the giant `mt X hp` chain.
- **Alternative: replace `mt X hp` chain with a small `intro`-style fact.** When the chain's sole purpose is to move `hp : ¬(P ↔ Q)` through normalization into `¬((P' = Q') = True)` (grind's normalized form), we could emit `have h_false : ¬<normalized> := fun h => hp (by grind_iff_eq_replay)` where `grind_iff_eq_replay` is a small re-derivation — but this requires generating tactic code that replays `Lean.Grind.iff_eq`, `Lean.Grind.Int.lt_eq`, `Int.Linear.norm_le` normalizations, which is essentially a partial reimplementation of grind's normalization pass.
