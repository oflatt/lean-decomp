...
- Specialized grind handling includes:
  ...
  - `tryArithLeafLia` collapses any arithmetic-shaped proof (LE/LT/Eq with `containsAutomationInternals`) into a single `lia` step. If `lia` does not close, walks the proof for `Iff.mp Finset.mem_*` references and inserts `rw [Finset.mem_*] at <hyp>` before retrying. Closes most grind polynomial-normalization leaves without re-elaborating their `eagerReduce` certificates;
  - `tryDecompFinsetIntervalMembership` for goals shaped `_ ∈ Finset.<X> a b`: emits `rw [Finset.mem_X]; refine ⟨?_, ?_⟩ <;> lia` (or `rw + lia` for half-open intervals). Uses the proof term to identify the right `mem_X` lemma when the goal's set is an opaque `set r := …` fvar;
...
- **The Sum L70/L36 timeouts are inherent to grind's polynomial reasoning, not to our decomposition.** Profiling a 12s `refine` shows `Add.add` unfolded ~312k times during type-checking. The leaves carry `eagerReduce (Eq.refl true)` polynomial-equality certificates, and the result types are grind-internal `Int.Linear.Expr.denote …` and `Lean.Grind.Order.le_unsat_k …` shapes — verifying these requires the same kernel reduction grind originally amortized internally. The original `with_unfolding_all exact <whole proof>` is also slow at this elaboration cost; our structural decomposition trades a single giant `exact` for many smaller `refine`/`exact` pieces, but the kernel still does the same polynomial work. Closing a leaf with `lia` is not faster either: `lia` *is* `cutsat` (now routed through `grind`), so on a goal it cannot close it spends its full heartbeat budget before giving up, and on grind-internal types each `whnf`/`isDefEq` triggers the same unfold cascade.
- **Decompilation does not share subterms.** Walking the proof as a tree (no hash-consing or `let`-binding) means an inner subproof that appears `n` times generates `n` independent occurrences in the output. For grind's polynomial-shape proofs, this multiplies the elaborator's work — each occurrence re-runs the same definitional reduction. CSE on the proof term before decompilation would amortize this, but is deferred until we have a clear motivating case where it changes the outcome (rather than just changing the elaboration time within an already-failing budget).
- **Within the structural-decompilation budget we have, the new specialized handlers reduce L70's generated tactic from ~1050 → ~460 lines and unblock several other shapes.** The wins land on goals where `lia` *can* close (the `tryDecompEqMpIntLinearNormLe` cases), where the iff transport reduces to a single `rw [Finset.mem_X]` (the `tryDecompFinsetIntervalMembership` cases), and where the proof's `And.intro` decomposes into independent component proofs (the `tryDecompAndIntro` + `simplifyProjOfIntro` chain). These don't change the L70 outcome — the remaining two `with_unfolding_all exact` lambdas dominate elaboration time — but they do clean up many intermediate leaves and benefit other `Int.lean` cases that were already passing.
...
- **The Sum L70/L36 blocker is fundamental, not incremental.** Adding more `lia`-or-similar leaf handlers doesn't fix it because `lia` IS `grind` (via cutsat) and pays the same kernel-reduction cost on the grind-internal types in the goal. Three options worth investigating, roughly in order of effort:
  - *CSE on the simplified proof term.* Hash-cons subterms with `refcount > 1`, emit `let h := <subterm>` bindings, substitute fvars in the main proof. The polynomial certificates and `RArray` contexts repeat throughout L70's proof; binding them would cut the kernel work proportionally. Requires a pre-pass on the Expr DAG before decompilation.
  - *Skip emitting `with_unfolding_all exact` when the leaf is small enough that the decomposed form would not increase work.* For mid-sized leaves (say <= 500 nodes), the `with_unfolding_all` mode is the cause of the slowness; without it, normal-transparency `exact` may succeed on the same term. The current `containsEagerReduce` heuristic forces `with_unfolding_all` whenever an `eagerReduce` appears anywhere — a tighter check (only when the `eagerReduce` is at a position the kernel would actually need to reduce) would help.
  - *Forall-membership preprocessor for `lia`.* When `lia` fails on an arithmetic leaf, walk the proof term for `<forall_hyp> <args>` patterns where `<forall_hyp>` is a lctx fvar whose type is `∀ y ∈ S, P y`. Synthesize `have <fresh> := <forall_hyp> <args>` before retrying `lia`. This was prototyped (`tryHavesPlusLia`, currently disabled) but the broader `tryArithLeafLia` it sat behind validated `lia` too aggressively across all arithmetic goals — each failed `lia` paid full heartbeats. Re-enabling it requires a trigger that only fires on shapes where the haves are likely to help (e.g. proof contains a fvar application of forall-shaped type).
